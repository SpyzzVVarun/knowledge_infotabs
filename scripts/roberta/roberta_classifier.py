...
def config(parser):
    ...
    parser.add_argument('--kg', default="none", type=str)
...
def train(args):
	...
	if args['embed_size'] == 768:
		...
		if(args["kg"]=="implicit"):
			model = RobertaModel.from_pretrained('roberta-large-mnli').cuda()
		else:
	...
	for ep in range(args['epochs']):
		...
		shutil.copy(f"/content/knowledge_infotabs/temp/models/drr/{full_name}","/content/drive/MyDrive/trial_weights")
